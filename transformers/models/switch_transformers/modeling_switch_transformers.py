...
def router_z_loss_func(router_logits: torch.Tensor) -> float:
    ...
    """
    # print(router_logits[0].shape,router_logits[1].shape)
    # num_groups = len(router_logits)
    num_groups,  tokens_per_group, _= router_logits.shape
    # print()
    # mid_1 = [torch.exp(a) for a in router_logits]
    # mid_2 = torch.cat([torch.sum(a, -1) for a in mid_1])
    # log_z = torch.log(mid_2) 
    log_z = torch.logsumexp(router_logits, dim=-1)
    z_loss = log_z**2
    return torch.sum(z_loss) / (num_groups * tokens_per_group)


def load_balancing_loss_func(router_probs: torch.Tensor, expert_indices: torch.Tensor, num_expert: int) -> float:
    r"""
...
class PyramidTransformersTop1Router(nn.Module):
    """
    Router using tokens choose top-1 experts assignment.
    ...
    def __init__(self, config: SwitchTransformersConfig):
        super().__init__()
        self.num_experts = len(config.require_experts[config.purne_layer.index(config.block_id)])
        self.expert_capacity = config.expert_capacity
        self.classifier = nn.Linear(config.hidden_size, len(config.require_experts[config.purne_layer.index(config.block_id)]), bias=config.router_bias)
        self.jitter_noise = config.router_jitter_noise
        self.ignore_padding_tokens = config.router_ignore_padding_tokens
        self.dtype = getattr(torch, config.router_dtype)
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, expert_class: nn.Module = SwitchTransformersDenseActDense):
        super().__init__()
        if config.pyramid==False or config.block_id not in config.purne_layer:
            ...
            self.experts = nn.ModuleDict()
            self.router = PyramidTransformersTop1Router(config)
            for idx in range(0,len(config.require_experts[config.purne_layer.index(config.block_id)])):
                self.experts[f"expert_{idx}"] = expert_class(config)
...
class SwitchTransformersBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, is_sparse=False,block_id=0):
        ...
        self.is_decoder = config.is_decoder
        self.is_sparse = is_sparse
        if self.is_decoder == True:
            config.block_id = block_id + 12
        else:
...
class SwitchTransformersPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SwitchTransformersLayerNorm):
            ...
            for idx in range(len(module.experts)):
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.num_expert = config.num_experts
...
class SwitchTransformersForConditionalGeneration(SwitchTransformersPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if output_router_logits:
                ...
                encoder_aux_loss = load_balancing_loss_func(encoder_router_probs, encoder_expert_indexes,self.encoder.num_expert)
                ...
                decoder_aux_loss = load_balancing_loss_func(decoder_router_probs, decoder_expert_indexes,self.encoder.num_expert)
    ...
    def _unpack_router_logits(self, router_outputs):
        ...
        num_expert = self.encoder.num_expert
        for router_output in router_outputs:
            if router_output[0] is not None:
                router_logits, expert_indexes = router_output
                total_expert_indexes.append(expert_indexes)
                if router_logits.shape[-1] != num_expert:
                    # length = router_logits.shape[-1]
                    # router_logits = router_logits.expand(-1,-1,total_router_logits[-1].shape[-1])
                    padded_matrix = torch.zeros((router_logits.shape[0],router_logits.shape[1],num_expert)).to('cuda:0')
                    padded_matrix[:,:,:router_logits.shape[2]] = router_logits
                    # router_logits[:,:,length:] = 0.
                    total_router_logits.append(padded_matrix)
                    continue
