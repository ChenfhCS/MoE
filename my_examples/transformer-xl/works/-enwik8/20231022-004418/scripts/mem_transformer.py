...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    ...
    def forward(self, inp, fuse_token=False):
        if self.pre_lnorm:
            ##### layer normalization + positionwise feed-forward
            core_out = super().forward(self.layer_norm(inp), fuse_token)
            ...
            ##### positionwise feed-forward
            core_out = super().forward(inp, fuse_token)
            core_out = self.dropout(core_out)
...
class RelPartialLearnableDecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, r, r_w_bias, r_r_bias, dec_attn_mask=None, mems=None, fuse_token=False):

        output = self.dec_attn(dec_inp, r, r_w_bias, r_r_bias,
                               attn_mask=dec_attn_mask,
                               mems=mems)
        output = self.pos_ff(output, fuse_token)
        ...
        if self.attn_type == 0: # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                core_out = layer(core_out, pos_emb, self.r_w_bias,
                        self.r_r_bias, dec_attn_mask=dec_attn_mask, mems=mems_i, fuse_token = self.fuse_token)
                hids.append(core_out)
        elif self.attn_type == 1: # learnable
            core_out = self.drop(word_emb)
            hids.append(core_out)
